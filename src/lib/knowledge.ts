...
export const PERSONAL: Personal = {
  ...
  strengths:
    "He does not rank himself as strongest in any one technology. He describes himself as early in his learning across the whole stack, and is currently reading into systems architecture. He would rather be judged on the projects than on a self-assessment — those are the actual evidence of what he can build.",
...
Write PLAIN TEXT ONLY. The chat window renders no markdown, so asterisks, hashes and backticks appear literally on screen and look broken. No **bold**, no headings, no code fences, no numbered or bulleted lists. Never begin a line with an asterisk, a hyphen, a bullet or a number followed by a full stop — those are list markers and they appear on screen exactly as typed. When you have several things to cover, give each one its own short paragraph of ordinary prose and let the blank line do the separating. Never use emoji.
